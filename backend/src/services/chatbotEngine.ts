...
export async function processWhatsAppMessage(message: ChatbotMessage): Promise<void> {
  ...
  if (messageType === 'audio') {
    if (!mediaUrl || !isTranscriptionConfigured()) {
      await sendWhatsAppMessage(company, from, '🎤 Voice received. Please type your message.');
      return;
    }

    await sendWhatsAppMessage(company, from, '🎤 Processing voice message…');

    const result = await transcribeWhatsAppVoice(
      mediaUrl,
    ...
    if (!result.success || !result.text) {
      await sendWhatsAppMessage(company, from, '❌ Unable to process voice message.');
      return;
    }

    input = result.text.toLowerCase();
  }
  ...
  if (session.step === 'otp') {
    if (buttonId === 'resend_otp') {
      await sendOTP(company, from, session.language);
      return;
    }

    const valid = await verifyOTP(from, input);
    if (!valid) {
      await sendWhatsAppButtons(company, from, t('otpInvalid', session.language), [
        { id: 'resend_otp', title: 'Resend OTP' }
      ]);
      return;
    }

    await sendWhatsAppMessage(company, from, t('otpVerified', session.language));
